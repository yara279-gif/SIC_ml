...
try:
    ...
    st.warning("Matplotlib not available - some visualizations disabled")
...
try:
    ...
    st.error("Joblib not available - model loading disabled")
...
def load_model():
    ...
    try:
        ...
        st.error("❌ Model file not found. Tried: " + ", ".join(model_paths))
        return None
...
def load_data():
    ...
    try:
        data_paths = [
            'smoke_detection_cleaned.csv',
            'data/smoke_detection_cleaned.csv',
            './smoke_detection_cleaned.csv'
        ]
        ...
        st.warning("⚠️ Using sample data. For full functionality, add smoke_detection_cleaned.csv")
...
def main():
    ...
    # Sidebar navigation - SIMPLIFIED to avoid errors
...
def show_realtime_detection(model):
    ...
    if preset_selected:
        ...
        st.success(f"✅ Loaded {preset_selected} scenario!")
            ...
            try:
                ...
                with col4:
                    if MATPLOTLIB_AVAILABLE:
                        ...
                        if risk_value > 0.7:
                            st.error(f"🔴 HIGH RISK: {risk_value:.1%}")
                        elif risk_value > 0.4:
                            st.warning(f"🟡 MEDIUM RISK: {risk_value:.1%}")
                        else:
                            st.success(f"🟢 LOW RISK: {risk_value:.1%}")
                ...
                risk_thresholds = {
                    'Temperature[C]': {'high': 30, 'medium': 20},
                    'Humidity[%]': {'high': 50, 'medium': 40},
                    'TVOC[ppb]': {'low': 5000, 'medium': 10000},
                    'eCO2[ppm]': {'low': 800, 'medium': 1200},
                    'Raw H2': {'high': 13100, 'medium': 13000},
                    'Raw Ethanol': {'low': 19000, 'medium': 19400},
                    'Pressure[hPa]': {'high': 939, 'medium': 938.5},
                    'PM1.0': {'high': 20, 'medium': 10},
                    'NC0.5': {'high': 60, 'medium': 30}
                }
                ...
                def get_feature_risk(feature_name, value):
                    ...
                    if feature_name in ['Temperature[C]', 'TVOC[ppb]', 'eCO2[ppm]', 'Raw Ethanol']:
...
def show_data_analysis(df):
    ...
    if 'Humidity_Category' in df.columns:
        df = df.drop(columns=['Humidity_Category'])
    
    # Sidebar for analysis options
    analysis_option = st.sidebar.selectbox(
        "Choose Analysis View:",
        ["Dataset Overview", "Target Analysis", "Feature Correlations", "Feature Distributions"]
    )
...
def show_correlation_analysis(df):
    ...
    if target_col in df.columns:
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            st.dataframe(target_correlations.sort_values(ascending=False))
...
def show_feature_distributions(df):
    ...
    if len(available_features) >= 4 and MATPLOTLIB_AVAILABLE:
        ...
        st.warning("Not enough key features available or matplotlib not available for distribution analysis.")
